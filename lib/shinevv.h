...
extern "C" {
	...
	* @param bMirror				是否镜像，一般本地图像为true，远端为false
	*/
	SHINEVV_INTERFACE_API void __stdcall SetScreenShareRenderWindow(
		void* handle,
		const char* pMemberId,
		void* pRenderWin,
		bool bMirror);
	...
	* @param handle						shinevv句柄
	*/
	SHINEVV_INTERFACE_API void __stdcall EnableScreenShare(
		void* handle);
	...
	* @param handle						shinevv句柄
	*/
	SHINEVV_INTERFACE_API void __stdcall DisableScreenShare(
		void* handle);
	...
	*/
	SHINEVV_INTERFACE_API void __stdcall SendUserMessage(
		void* handle,
		const char* data);
	...
	*/
	SHINEVV_INTERFACE_API void __stdcall GetUserMessageHistory(
		void* handle,
