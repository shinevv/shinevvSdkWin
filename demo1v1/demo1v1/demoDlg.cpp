...
BOOL CdemoDlg::OnInitDialog()
{
	...
	m_ctlVideoSizeList.AddString(_T("320x240"));
	m_ctlVideoSizeList.AddString(_T("640x360"));
	...
	m_ctlVideoSizeList.SetCurSel(0);
...
void CdemoDlg::Reset() {
	...
	m_ctlRemoteVideo.ShowWindow(FALSE);
...
LRESULT CdemoDlg::OnMessageRefreshMemberVideo(WPARAM wParam, LPARAM lParam) {
	...
	if (strMemberId.compare(m_strLocalMemberId) == 0) {
		...
		MessageBox(_T("切换全屏，请稍候。。。"),_T("1v1演示切换"), MB_OK);
		DisableMemberVideo(m_shinevv, m_strLocalMemberId.c_str(), NULL);
		SetRenderWindow(m_shinevv, strMemberId.c_str(), (void*)m_ctlLocalVideo.GetSafeHwnd(), true);
		EnableMemberVideo(m_shinevv, m_strLocalMemberId.c_str(), NULL);
		m_ctlLocalVideo.Invalidate();
	}
	else if (strMemberId.compare(m_strRemoteMemberId) == 0){
		SetTimer(100, 2000, NULL);
		MessageBox(_T("切换全屏，请稍候。。。"), _T("1v1演示切换"), MB_OK);
		DisableMemberVideo(m_shinevv, m_strRemoteMemberId.c_str(), NULL);
		SetRenderWindow(m_shinevv, strMemberId.c_str(), (void*)m_ctlRemoteVideo.GetSafeHwnd(), false, OnRecvVideoResolution);
		EnableMemberVideo(m_shinevv, m_strRemoteMemberId.c_str(), NULL);
		m_ctlRemoteVideo.Invalidate();
	}
...
void CdemoDlg::OnTimer(UINT_PTR nIDEvent)
{
	if (nIDEvent == 100)
	{
		HANDLE hWnd = ::FindWindowEx(NULL, NULL, NULL, L"1v1演示切换"); 
		if (hWnd)
		{
